...
func _perform_ai_decisions() -> void:
	if _is_ball_carried_by_opponent() and player.position.distance_to(ball.position) <= TACKLE_DISTANCE and randf() < TACKLE_PROBABILITY:
...
func _is_ball_carried_by_opponent() -> bool:
